...
Dim sEventRaces
Dim sngMaxY
Dim Finishers(), Registered(), Difference()
Dim Chart
...
ReDim Registered(1, UBound(Years) - 1)
ReDim Difference(1, UBound(Years) - 1)
...
If CLng(lEventID) > 0 Then
    ...
    For j = 0 To UBound(Years) - 1
        ...
        Registered(0, j) = Years(j)
        Registered(1, j) = "0"

        Difference(0, j) = Years(j)
        Difference(1, j) = "0"
...
If UBound(Finishers, 2) > 0 Then
    For i = 0 To UBound(Finishers, 2)
        ...
        Chart.AddPoint Registered(0, i), Registered(1, i), "000000", "Registered"
        Chart.AddLineGraphText Finishers(1, i), Finishers(0, i), Finishers(1, i), 0

        Chart.AddPoint Difference(0, i), Difference(1, i), "003399", "Difference"
        Chart.AddLineGraphText Finishers(1, i), Finishers(0, i), Finishers(1, i), 0
    Next
End If
