...
Dim ExclSites(), ExclMeets(), TempArr(7), SelUsers(), MeetRslts(), SiteRslts(), BegDates(), EndDates(), MeetSites()
...
If Request.Form.Item("submit_filters") = "submit_filters" Then  
    ...
    If IsArray(Meets) Then 
        ...
        For i = 0 To UBound(MeetSites) - 1
            If Request.Form.Item("site_" & MeetSites(i)) = "on" Then sExclSites = sExclSites & "'" & MeetSites(i) & "',"
        Next
        ...
        For i = 0 To UBound(Meets, 2) - 1
            If Request.Form.Item("meet_" & Meets(0, i)) = "on" Then sExclMeets = sExclMeets & Meets(0, i) & ","
        Next
...
ReDim ExclSites(0)
If Not sExclSites = vbNullString Then
    j = 0
    If Not CStr(sExclSites) = vbNullString Then
	    For i = 1 To Len(sExclSites)
		    If Mid(sExclSites, i, 1) = "," Then
			    ExclSites(j) = Trim(CStr(ExclSites(j)))
			    j = j + 1
			    ReDim Preserve ExclSites(j)
		    Else
			    If i = Len(sExclSites) Then
			        ExclSites(j) = Trim(CStr(ExclSites(j)))
			        j = j + 1
			        ReDim Preserve ExclSites(j)
                Else
                    ExclSites(j) = ExclSites(j) & Mid(sExclSites, i, 1)
                End If
		    End If
	    Next
    End If
...
Private Sub MyResults(lThisMmbr)
    ...
    If Len(sMeetIDs) > 1 Then
        Set rs = Server.CreateObject("ADODB.Recordset")
        sql = "SELECT ir.MeetsID, ir.Gate, ir.Compress, r.RaceDesc, r.RaceDist, r.RaceUnits, ir.RacesID, ir.RaceTime FROM IndRslts ir INNER JOIN Races r ON r.RacesID = ir.RacesID "
        sql = sql & "WHERE ir.RosterID = " & lThisMmbr & " AND ir.Place > 0 AND ir.MeetsID IN (" & sMeetIDs & ")"
        rs.Open sql, conn2, 1, 2
        ...
        'RsltsArr(1, x) = dMeetDate
        'RsltsArr(2, x) = sSite
        'RsltsArr(3, x) = race desc
        'RsltsArr(4, x) = race dist
        'RsltsArr(5, x) = race units
        'RsltsArr(6, x) = GetPlace(rs(6).Value, lThisMmbr)   'get place (racesid,lthismmbr)
        'RsltsArr(7, x) = race time

        If IsArray(RsltsArr) Then
            For x = 0 To UBound(RsltsArr, 2)
                Call MeetData(RsltsArr(6, x))

                RsltsArr(0, x) = sMeetName
                RsltsArr(1, x) = dMeetDate
                RsltsArr(2, x) = sSite
                RsltsArr(6, x) = GetPlace(RsltsArr(6, x), lThisMmbr)   'get place (racesid,lthismmbr)
                RsltsArr(7, x) = ConvertToSeconds(RsltsArr(7, x))
            Next
            ...
            Select Case sSortBy
                ...
                Case "perf"
                    For x = 0 To UBound(RsltsArr, 2) - 1
                        For y = x + 1 To UBound(RsltsArr, 2)
                            If CSng(RsltsArr(7, x)) > CSng(RsltsArr(7, y)) Then
                                For z = 0 To 7
                                    TempArr(z) = RsltsArr(z, x)
                                    RsltsArr(z, x) = RsltsArr(z, y)
                                    RsltsArr(z, y) = TempArr(z)
            ...
            'convert to minutes
            For x = 0 To UBound(RsltsArr, 2)
                RsltsArr(7, x) = ConvertToMinutes(Round(RsltsArr(7, x), 1))
            Next
        End If
    End If
End Sub
...
Private Sub GetMeetRslts(lThisMeet)
    'get the results for the selected meet
    Dim x, y, z

    Set rs = Server.CreateObject("ADODB.Recordset")
    sql = "SELECT r.RosterID, r.Gender, r.RaceDesc, r.RaceDist, r.RaceUnits, ir.RacesID, ir.RaceTime FROM IndRslts ir INNER JOIN Races r ON r.RacesID = ir.RacesID WHERE ir.MeetsID = " 
    sql = sql & lThisMeet & " AND ir.RosterID In (" & sRosterIDs & ") AND ir.Place > 0"
    rs.Open sql, conn2, 1, 2
    If Not rs.EOF Then MeetRslts = rs.GetRows()
    rs.Close
    Set rs = Nothing

    'MeetRslts(0, x) = 'last name...need to be changed...to whole name?
    'MeetRslts(1, x) = 'meet name
    'MeetRslts(2, x) = 'race desc
    'MeetRslts(3, x) = 'race dist
    'MeetRslts(4, x) = 'race units
    'MeetRslts(5, x) = 'place...use functiom GetPlace(raceid, roster)
    'MeetRslts(6, x) = 'race time

    If IsArray(MeetRslts) Then
        For x = 0 To UBound(MeetRslts, 2)
            MeetRslts(0, x) = MyName(MeetRslts(0, x))
            MeetRslts(1, x) = MeetName(lThisMeet)
        Next
    End If

    For x = 0 To UBound(MeetRslts, 2) - 1
        For y = x + 1 To UBound(MeetRslts, 2)
...
Private Sub GetSiteRslts(sThisSite)
    ...
    For z = 0 To UBound(SiteMeets, 2) - 1
        For y = 0 To UBound(ViewPerf, 2) - 1
            ...
            sql = sql & SiteMeets(0, z) & " AND ir.RosterID = " & ViewPerf(0, y) & " AND ir.Place > 0 ORDER BY ir.RacesID, ir.RaceTime"
	        rs.Open sql, conn2, 1, 2
	        Do While Not rs.EOF
...
Private Sub MeetData(lThisRace)
	...
	Set rs2 = Server.CreateObject("ADODB.Recordset")
	sql2 = "SELECT m.MeetName, m.MeetDate, m.MeetSite FROM Meets m INNER JOIN IndRslts ir ON m.MeetsID = ir.MeetsID "
    sql2 = sql2 & "WHERE ir.RacesID = " & lThisRace 
   	rs2.Open sql2, conn2, 1, 2
...
Private Function MyName(lThisRosterID)
	Set rs2 = Server.CreateObject("ADODB.Recordset")
	sql2 = "SELECT FirstName, LastName FROM Roster WHERE RosterID = " & lThisRosterID 
   	rs2.Open sql2, conn2, 1, 2
    MyName = Replace(rs2(1).Value, "''", "'") & ", " & Replace(rs2(0).Value, "''", "'")
	rs2.Close
	Set rs2 = Nothing
End Function

Private Function MeetName(lMeetID)
	Set rs2 = Server.CreateObject("ADODB.Recordset")
	sql2 = "SELECT MeetName FROM Meets WHERE MeetsID = " & lMeetID 
   	rs2.Open sql2, conn2, 1, 2
    MeetName = Replace(rs2(0).Value, "''", "'")
	rs2.Close
	Set rs2 = Nothing
End Function
%>
