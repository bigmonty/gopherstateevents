...
Private Sub RaceResults(lRaceID, sGender)
	...
	sql = sql & "WHERE ir.RaceID = " & lRaceID & " AND p.Gender = '" & sGender & "' ORDER BY ir.FnlScnds"
    rs.Open sql, conn, 1, 2
	Do While Not rs.EOF
		IndRslts(0, x) = rs(0).Value & "-" & Replace(rs(1).Value, "''", "'") & ", " & Replace(rs(2).Value, "''", "'")
...
<%For j = 0 To UBound(Races, 2) - 1%>
    <div style="float: left;width: 600px;margin: 0;padding: 0;font-size: 1.2em;">
    ...
    <div style="margin: 0 0 0 625px;padding: 0;font-size: 1.2em;">
