...
Dim i, m, n, x
Dim sRaceName, sThisDist, sThisLeg, sThisSplit, sThisTrans, sTimingMethod, sChipStart, sEventClass, sDist, sAllowDuplAwds
Dim sEventRaces, sShowAge, sEventName, sSortRsltsBy, sGender
...
If CStr(iEventType) = vbNullString Then iEventType = 0
...
sFileName = "C:\Inetpub\h51web\gopherstateevents\dwnlds\" & sEventName & "_" & sRaceName & "-Combined_"
sFileName = sFileName & Year(dEventDate) & ".txt"
Set fname=fs.CreateTextFile(sFileName, True)

fname.WriteLine("GSE Race Results for " & sEventName & " " & sRaceName & "-Combined  on " & dEventDate)
fname.WriteLine("Generated: " & Now())
fname.WriteBlankLines(1)
If sShowAge = "y" Then
    fname.WriteLine("PL" & vbTab & " BIB" & vbTab & "NAME" & Space(26) & vbTab & "AGE" & vbTab & "MF" & vbTab & "CHIP TIME" & vbTab & "GUN TIME" & Space(3) & vbTab & "START TIME" & Space(3) & vbTab & "CITY" & Space(14) & vbTab & "ST")
Else
    fname.WriteLine("PL" & vbTab & " BIB" & vbTab & "NAME" & Space(26) & vbTab & "AGE GRP" & vbTab & "MF" & vbTab & "CHIP TIME" & vbTab & "GUN TIME" & Space(3) & vbTab & "START TIME" & Space(3) & vbTab & "CITY" & Space(14) & vbTab & "ST")
End If

For i = 0 to UBound(IndRslts, 2)
	sDwnldName = IndRslts(2, i) & " " & IndRslts(1, i)
	If Len(sDwnldName) < 30 Then
		sDwnldName = sDwnldName & Space(30 - Len(sDwnldName))
	Else
		sDwnldName = Left(sDwnldName, 30)
	End If
...
'	End If
	...
	End If
	fname.WriteLine(i + 1 & vbTab & IndRslts(0, i) & vbTab & sDwnldName & vbTab & IndRslts(4, i) & vbTab & IndRslts(3, i) & vbTab & IndRslts(5, i) & vbTab & sDwnldPerMi & vbTab & sDwnldPerKM & vbTab & sDwnldCity & vbTab & IndRslts(9, i))
...
'begin download
Response.Redirect "/dwnlds/" & sEventName & "_" & sRaceName & "-Combined_" & Year(dEventDate) & ".txt"
%>
<!DOCTYPE html>
<html>
<head>
<!--#include file = "../../includes/meta2.asp" -->
<title>GSE Download Combined Results</title>
<meta name="description" content="Gopher State Events (GSE) Combined Results Download.">
