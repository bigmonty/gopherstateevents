...
sql = "SELECT EventName, EventDate, BlockImage FROM FeaturedEvents WHERE FeaturedEventsID = " & lFeaturedEventID
rs.Open sql, conn, 1, 2
sEventName = Replace(rs(0).Value, "''", "'")
dEventDate = rs(1).Value
sBlockImage = rs(2).Value
rs.Close
Set rs = Nothing
%>
<html lang="en">
...
<title>GSE Featured Event Block Image Upload</title>
...
<div class="container">
	<h3 class="h3">Featured Event Block Image Upload:<br><%=sEventName%> on <%=dEventDate%></h3>
		
    <h4 class="h4">Existing Image:</h4>	
    <%If sBlockImage & "" = "" Then%>
        <p>No image available.</p>
    <%Else%>
        <img src="/featured_events/images/<%=sBlockImage%>" alt="Banner Image" class="img-responsive">
    <%End If%>
	
    <br>

    <h4 class="h4">Upload New Image:</h4>
	<form name="upload" method="Post" action="receive_block.asp" enctype="multipart/form-data">
