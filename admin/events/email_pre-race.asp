...
ElseIf Request.Form.Item("submit_sample") = "submit_sample" Then
    Dim lRaceID
    Dim sSampleRecips
    ...
    sSampleRecips = Request.Form.Item("sample_recips")
    ...
    sPageToSend = "http://www.gopherstateevents.com/misc/pre_race.asp?event_id=" & lEventID & "&race_id=" & lRaceID & "&part_id=1&supp_msg=" & sSuppMsg
...
'		.To = sSampleRecips
		...
		.From = "bob.schneider@gopherstateevents.com"
   		.Subject = "Information for " & sEventName & " (sample)"
...
ElseIf Request.Form.Item("submit_ind") = "submit_ind" Then
    ...
    sSuppMsg = Request.Form.Item("supp_msg_ind")
    If Not sSuppMsg = vbNullString Then sSuppMsg = Replace(sSuppMsg, chr(34), "'")
		...
		<div class="col-md-10">
			...
			<%If Not Clng(lEventID) = 0 Then%>
			    ...
			    <div class="row bg-info">
                    <h4 class="h4">Send Sample</h4>
				    <form name="sample_email" Method="Post" action="email_pre-race.asp?event_id=<%=lEventID%>">
					<div class="form-group">
						<label for="supp_msg_sample">Supplemental Message:</label>
						<textarea class="form-control" name="supp_msg_sample" id="supp_msg_sample" rows="3"></textarea>
						<label for="sample_recips">Send To:</label>
						<textarea class="form-control" name="sample_recips" id="sample_recips" rows="2">bob.schneider@gopherstateevents.com</textarea>
						<input type="hidden" name="submit_sample" id="submit_sample" value="submit_sample">
						<input class="form-control" type="submit" name="submit1" id="submit1" value="Send Sample">
				    </div>
					</form>
			    </div>
                ...
                <div style="padding:5px;">
					...
					<form class="form" role="form" name="send_ind" method="Post" action="email_pre-race.asp?event_id=<%=lEventID%>">
			        <div class="form-group">
                       <label for="supp_msg_ind">Supplemental Message: </label>
						<textarea class="form-control" name="supp_msg_ind" id="supp_msg_ind" rows="2"></textarea>
				        <input type="hidden" name="submit_ind" id="submit_ind" value="submit_ind">
				        <input class="form-control" type="submit" name="submit4" id="submit4" value="Send Selected">
 			        </div>
			        <%For i = 0 To UBound(RaceArr, 2) - 1%>
				        <%Call GetRaceInfo(RaceArr(0, i))%>
					
                        <h4 class="h4"><%=RaceArr(1, i)%></h4>
