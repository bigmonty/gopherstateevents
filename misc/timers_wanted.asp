...
If Request.form.Item("submit_this") = "submit_this" Then
	...
	sql = sql & "' AND WhenHit >= '" & Now() - CSng(1/72) & "' AND Page = 'timers_wanted' ORDER BY AuthAccessID DESC"
	rs.Open sql, conn, 1, 2
	If rs.RecordCount > 0 Then Session("access_timers_wanted") = "y"
	rs.Close
	Set rs = Nothing

	If Session("access_timers_wanted") = "y" Then	'if they are an authorized user allow them to proceed
...
'        If sHackMsg = vbNullString Then lGSEEventID = CleanInput(Request.Form.Item("gse_event")) 
...
Else
	...
	Session.Contents.Remove("access_timers_wanted")
End If
	...
	<div id="row">
        ...
        <div class="col-md-5 bg-info">
            <p class="bg-danger text-danger">
                NOTE:  If you are a GSE Event Director please log in and use that form under your profile.  It will be much
                easier for you to create a featured event there.
            </p>
			...
			<%Else%>
                ...
                <%End If%>
				<form class="form" name="request_feature" method="post" action="timers_wanted.asp" onSubmit="return chkFlds();">
